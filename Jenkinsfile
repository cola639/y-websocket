    ...
    stages {
      ...
      steps {
        script {
                    ...
                    // 运行新的容器并指定网络（如果网络存在）
                    if (networkExists) {
                        sh "docker run -d -p 80 -p 1235:1235 --name ${DOCKER_CONTAINER_NAME} --network ${DOCKER_NETWORK} ${DOCKER_IMAGE}"
                    } else {
                        sh "docker run -d -p 80 -p 1235:1235 --name ${DOCKER_CONTAINER_NAME} ${DOCKER_IMAGE}"
                    }
        }
      }
        }
